...
function parseStockData(data) {
  ...
  if (data.technicalIndicators && data.technicalIndicators.data) {
    ...
    parsed.technical = {
      ma10hr: techs?.movingAverage10hr || 0,
      ma24hr: techs?.movingAverage24hr || 0, 
      rsi: techs?.RSI || 50,
      macd: techs?.MACD?.MACD || 0,
      macdSignal: techs?.MACD?.signal || 0,
      macdHistogram: techs?.MACD?.histogram || 0,
      atr: techs?.ATR || 0,
      ema10hr: techs?.EMA10hr || 0,
      bbMiddle: techs?.BBANDS?.middle || 0,
      bbUpper: techs?.BBANDS?.upper || 0, 
      bbLower: techs?.BBANDS?.lower || 0,
      bbPB: techs?.BBANDS?.pb || 0.5,
      vwap: parseFloat(techs?.VWAP || '0'),
      adx: techs?.ADX?.adx || 0,
      pdi: techs?.ADX?.pdi || 0,
      mdi: techs?.ADX?.mdi || 0,
      stochK: techs?.STOCH?.k || 50,
      stochD: techs?.STOCH?.d || 50,
      cci: techs?.CCI || 0,
      obv: parseInt((techs?.OBV || '0').replace(/,/g, '')),
      mfi: techs?.MFI || 50,
      ema5: techs?.EMA5 || 0,
      ema9: techs?.EMA9 || 0,
      ema20: techs?.EMA20 || 0,
